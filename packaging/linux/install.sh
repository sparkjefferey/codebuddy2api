...
#
# 会复制本目录到 ~/.local/opt/workbuddy-gateway ，并 enable --user 服务。
#
set -euo pipefail

SRC="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"   # 本压缩包解压目录
APP="$HOME/.local/opt/workbuddy-gateway"
UNIT="$HOME/.config/systemd/user/workbuddy-gateway.service"
UNIT_SRC="$SRC/workbuddy-gateway.service"
...
if [[ "${1:-}" == "uninstall" ]]; then
  systemctl --user stop workbuddy-gateway 2>/dev/null || true
  systemctl --user disable workbuddy-gateway 2>/dev/null || true
  rm -f "$UNIT"
...
sleep 1
if systemctl --user is-active --quiet workbuddy-gateway; then
  echo "✅ 网关已安装并运行: ~/.local/opt/workbuddy-gateway"
  echo "   控制台: http://127.0.0.1:8787"
  echo "   状态  : systemctl --user status workbuddy-gateway"
  echo "   日志  : tail -f $APP/gateway.log"
  echo "   卸载  : $APP/install.sh uninstall"
else
  echo "⚠️  服务未启动，请 systemctl --user status workbuddy-gateway 排查。" >&2
