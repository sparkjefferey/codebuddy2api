...
- **渠道指纹中和（内置）**：上游风控会扫描请求内容中的官方客户端签名（11128 Illegal API invocation），且指纹不限于品牌词（CC 环境样板即可触发）。网关出站前把 Claude Code/Codex 的系统样板压缩为一句中性摘要（Python 版 compact_harness 同款策略），并对残余文本中的品牌词插零宽空格兜底；同时为未带 system 的请求补首条 system 消息（workbuddy.ai 后端强制要求）
