...
- **渠道指纹中和（内置）**：上游风控会扫描请求内容中的官方客户端签名（11128 Illegal API invocation），且指纹不限于品牌词（CC 环境/对话里的 "Main branch (you will usually use this for PRs): …" 一行即可触发，assistant 文本尤甚）。网关出站前剪除 CC/Codex 壳层的机器上下文快照，并对品牌词与已知指纹短语插零宽空格拆分；同时为未带 system 的请求补首条 system 消息（workbuddy.ai 后端强制要求）
